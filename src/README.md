...
`rows` y `columns` son obligatorias. Todo lo demás tiene valor por defecto o es opcional, `rowKey`
incluida: conviene declararla siempre y es indispensable en modo servidor (ver
[Si no tienes un identificador](#si-no-tienes-un-identificador)).
...
| `rowKey`                | `keyof TRow \| ((row: TRow, index: number) => string \| number)` | por referencia del objeto     | Identidad de una fila. Se estampa como `data-row-key` para que el DOM siga siendo inspeccionable y testeable. Nunca afecta al reciclado: el pool recicla por slot de viewport. Sin ella, la identidad es la referencia de cada objeto, que **no alcanza en modo servidor**. |
...
| `.dt-header-cell`      | `role="columnheader"`, `aria-colindex` (base 1 sobre las columnas **visibles**), `aria-sort` solo en las columnas con `sortable`                                                 |
...
**`aria-sort` solo lo llevan las columnas ordenables.** Una columna con `sortable` lo lleva
siempre: `ascending` o `descending` mientras participa del orden, y `none` cuando no. Una columna
sin `sortable` no lo lleva **nunca**, ni siquiera como `none`, y no es un olvido: `aria-sort="none"`
le anuncia a un lector de pantalla que esa columna se puede ordenar, y anunciarla ordenable donde no
se puede sería peor que no anunciar nada. `sortable: 'menu'` cuenta como ordenable: se ordena desde
otro lado, pero se ordena. Con varios criterios, cada columna declara su propio sentido y nada más:
la prioridad —el número junto a la flecha— no tiene atributo ARIA que la exprese, y el indicador va
con `aria-hidden`, así que un lector de pantalla no la anuncia. Ver [Ordenamiento](#ordenamiento).
...
**La contrapartida: la celda activa no le llega a un lector de pantalla.** Con el foco quieto en el
viewport y sin `aria-activedescendant` que apunte a la celda, moverse con las flechas cambia la
marca visual y el `aria-selected` de una celda —o de una fila, en modo fila— que no tiene el foco, y
ninguna de las dos cosas se anuncia. Tampoco hay una región `aria-live` para lo que pasa sin mover
el foco —ordenar, copiar un rango, plegar un grupo—. Está en [Limitaciones](#limitaciones).
...
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Filtrado y búsqueda**                         | Ni interfaz ni helper: el orden trae [`sortRows`](#en-memoria-dos-líneas) para el caso en memoria, y filtrar no tiene equivalente. Se filtra aguas arriba y se pasa el array ya filtrado; en modo servidor, se cambia la consulta y se pone `rows = []` (ver [Ordenar y filtrar](#ordenar-y-filtrar)).                                                                                                                     |
...
| **Vaciar con `Supr` o `Retroceso`**             | Ninguna de las dos teclas hace nada. Vaciar una celda es abrir el editor y borrar a mano, y un rango no se vacía de una vez: como el pegado, tendría que pasar por la tubería de edición celda por celda.                                                                                                                                                                                                                  |
| **Rechazar un valor al confirmar**              | `beforeEdit` impide que el editor se **abra**; una vez abierto, los editores incorporados no tienen dónde validar. `editCommit` no puede devolver el editor ni marcar un error: no escribir el valor en `rows` lo descarta, y la celda vuelve a mostrar el anterior sin decir por qué. Con `editor: 'slot'` sí se puede, porque el componente propio decide cuándo llamar a `commit()`.                                    |
| **Deshacer y rehacer**                          | No hay historial ni `Ctrl`+`Z`. La tabla nunca escribe en `rows`, así que el historial le corresponde a quien escribe: el handler de `editCommit`.                                                                                                                                                                                                                                                                         |
| **Redimensionar columnas con el teclado**       | El tirador lleva `role="separator"`, pero no es enfocable, no responde a teclas y no lleva `aria-valuenow`. `columnWidths` queda como el camino para una UI propia.                                                                                                                                                                                                                                                        |
| **Autoajustar el ancho al contenido**           | El doble clic sobre el tirador no hace nada. El ancho sale solo de `width`, `columnWidths` o el arrastre.                                                                                                                                                                                                                                                                                                                  |
| **Encabezados agrupados**                       | Hay una sola fila de encabezado: no hay títulos que abarquen varias columnas.                                                                                                                                                                                                                                                                                                                                              |
| **Fila de totales y filas ancladas**            | Los agregados solo se dibujan en las cabeceras de grupo. No hay una fila de totales generales al pie, ni filas fijas arriba o abajo del cuerpo.                                                                                                                                                                                                                                                                            |
| **Detalle desplegable por fila**                | Una fila no se expande para mostrar contenido propio debajo; lo único que se pliega son los grupos.                                                                                                                                                                                                                                                                                                                        |
| **Datos en árbol**                              | La agrupación arma el árbol a partir de los **valores** de unas columnas, no de relaciones padre-hijo entre filas. Una jerarquía propia de los datos no tiene cómo expresarse.                                                                                                                                                                                                                                             |
| **Reordenar filas arrastrando**                 | Solo se arrastran las columnas.                                                                                                                                                                                                                                                                                                                                                                                            |
| **Menú contextual**                             | No hay menú de clic derecho, ni eventos de `contextmenu` o de doble clic sobre una celda o una fila: el doble clic está tomado por la edición.                                                                                                                                                                                                                                                                             |
| **Exportar a CSV o Excel**                      | Lo más cercano es `Ctrl`+`C`, que escribe TSV en el portapapeles y solo lo seleccionado (ver [El copiado](#el-copiado)).                                                                                                                                                                                                                                                                                                   |
| **Escritura de derecha a izquierda**            | Sin soporte para `dir="rtl"`: el anclaje al inicio, la regleta de numeración y las flechas del teclado dan por hecho que el inicio es la izquierda.                                                                                                                                                                                                                                                                        |
| **Celda activa para tecnologías de asistencia** | El foco se queda en el viewport y no hay `aria-activedescendant`, así que moverse con las flechas no le da a un lector de pantalla nada que anunciar. Tampoco hay una región `aria-live`: ordenar, copiar un rango o plegar un grupo no producen ningún anuncio. Ver [Accesibilidad](#accesibilidad).                                                                                                                      |
| **Pantallas táctiles**                          | Sin tratamiento propio: el único ajuste es el `touch-action` del tirador de ancho. Seleccionar un rango o mover una columna con el dedo no está probado.                                                                                                                                                                                                                                                                   |
